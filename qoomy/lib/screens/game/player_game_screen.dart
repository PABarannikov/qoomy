...
class _PlayerGameScreenState extends ConsumerState<PlayerGameScreen> {
      ...
      children: [
        ...
        Expanded(
          child: chatAsync.when(
            data: (messages) => _buildChatSection(messages, currentUser?.id, room),
            loading: () => const Center(child: CircularProgressIndicator()),
            error: (e, _) => Center(child: Text('Error: $e')),
          ),
        ),
  ...
  Widget _buildChatSection(List<ChatMessage> messages, String? currentUserId, RoomModel room) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Send an answer or comment below',
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildMessageBubble(message, isMe, isAiMode: isAiMode);
      },
    );
  }

  Widget _buildMessageBubble(ChatMessage message, bool isMe, {bool isAiMode = false}) {
